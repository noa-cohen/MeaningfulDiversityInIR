...
class mask_generator:
    def __init__(self, mask_type, mask_len_range=None, mask_prob_range=None,
                 image_size=256, margin=(16, 16), mask_path=None):
        """
    ...
    def __call__(self, img, fnames):
        if self.mask_type == 'random':
            ...
            for i in range(img.shape[0]):
                jpgpath = os.path.join(self.mask_path, f'{fnames[i]}.jpg')
                jpegpath = os.path.join(self.mask_path, f'{fnames[i]}.JPEG')
                if os.path.exists(jpgpath):
                    m = np.array(Image.open(jpgpath))
                elif os.path.exists(jpegpath):
                    m = np.array(Image.open(jpegpath))
                else:
                    raise ValueError(f"No such mask path {jpgpath}")

                m = m[:, :, 0]
                m = (m > 127) * 1
                mask.append(torch.from_numpy(m).unsqueeze(0))
            return torch.stack(mask).to(img.device)
...
def create_gaussian(size, sigma1, sigma2=-1, is_tensor=False):
    ...
    func2 = func1 if sigma2 == -1 else [np.exp(-z ** 2 / (2 * sigma2 ** 2)) / np.sqrt(2 * np.pi * sigma2 ** 2)
                                        for z in range(-size // 2 + 1, size // 2 + 1)]
