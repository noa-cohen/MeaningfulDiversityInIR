...
class CS(A_functions):
    ...
    def Vt(self, vec):
        ...
        recon[:, :self.channels * self.y_dim ** 2 * self.cs_size] = patches[:, :, :, :self.cs_size
                                                                            ].contiguous().reshape(
                                                                                vec.shape[0], -1)
        recon[:, self.channels * self.y_dim ** 2 * self.cs_size:] = patches[:, :, :, self.cs_size:
                                                                            ].contiguous().reshape(
...
class Inpainting(A_functions):
    def __init__(self, channels, img_dim, missing_indices, device):
        ...
        self.kept_indices = torch.Tensor([i for i in range(channels * img_dim**2) if i not in missing_indices]
                                         ).to(device).long()
...
class SuperResolution(A_functions):
    ...
    def V(self, vec):
        ...
        for idx in range(self.ratio**2-1):
            patches[:, :, :, idx+1] = temp[:, (self.channels*self.y_dim**2+idx)::self.ratio**2-1
    ...
    def Lambda_noise(self, vec, a, sigma_y, sigma_t, eta, epsilon):
        ...
        if a != 0 and sigma_y != 0:
            ...
            change_index = (sigma_t > a * sigma_y * inverse_singulars) * 1.0
            d1_t = d1_t * (-change_index + 1.0) + torch.sqrt(change_index *
                                                             (sigma_t ** 2 -
                                                              a ** 2 * sigma_y ** 2 * inverse_singulars ** 2))
            d2_t = d2_t * (-change_index + 1.0)
...
class Colorization(A_functions):
    ...
    def V(self, vec):
        ...
        needles = torch.matmul(self.V_small, needles.reshape(-1, self.channels, 1)).reshape(
            vec.shape[0], -1, self.channels)  # shape: B, WA, C
    ...
    def Lambda(self, vec, a, sigma_y, sigma_t, eta):
        ...
        needles = torch.matmul(self.Vt_small, needles.reshape(-1, self.channels, 1)
                               ).reshape(vec.shape[0], -1, self.channels)
        ...
        needles = torch.matmul(self.V_small, needles.reshape(-1, self.channels, 1)
                               ).reshape(vec.shape[0], -1, self.channels)
    ...
    def Lambda_noise(self, vec, a, sigma_y, sigma_t, eta, epsilon):
        ...
        needles_vec = torch.matmul(self.V_small, needles_vec.reshape(-1, self.channels, 1)
                                   ).reshape(vec.shape[0], -1, self.channels)
        recon_vec = needles_vec.permute(0, 2, 1).reshape(vec.shape[0], -1)

        needles_epsilon = torch.matmul(self.V_small, needles_epsilon.reshape(-1, self.channels, 1)
                                       ).reshape(vec.shape[0], -1, self.channels)
...
class SRConv(A_functions):
    ...
    def __init__(self, kernel, channels, img_dim, device, stride=1):
        ...
        self._perm = torch.Tensor([self.img_dim * i + j for i in range(self.small_dim)
                                   for j in range(self.small_dim)] +
                                  [self.img_dim * i + j for i in range(self.small_dim)
                                   for j in range(self.small_dim, self.img_dim)]).to(device).long()
...
class Deblurring(A_functions):
    ...
    def __init__(self, kernel, channels, img_dim, device, ZERO=3e-2):
        ...
        self.singulars_small[self.singulars_small < ZERO] = 0
        # calculate the singular values of the big matrix
        self._singulars_orig = torch.matmul(self.singulars_small_orig.reshape(img_dim, 1),
                                            self.singulars_small_orig.reshape(1, img_dim)).reshape(img_dim**2)
