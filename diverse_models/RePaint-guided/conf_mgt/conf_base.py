...
def write_images(imgs, img_names, dir_path):
    sub_dirs = [os.path.join(dir_path, '_'.join(d.split('.')[0].split('_')[1:]), "images") for d in img_names]
    for sub in sub_dirs:
        os.makedirs(sub, exist_ok=True)

    for image_name, image, sub in zip(img_names, imgs, sub_dirs):
        out_path = os.path.join(sub, image_name)
