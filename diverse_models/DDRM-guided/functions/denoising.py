import torch
import torch.optim as optim  # 4guidance
...
def compute_sigma(beta_t, a_t, a_tnext):
    s_squared = beta_t * (1.0 - a_tnext) / (1.0 - a_t)
    return s_squared.sqrt()
...
def get_nn(x):
    similarity_mat = get_l2_similarity_matrix(x)
...
def get_perp(x):
    xperp = torch.zeros_like(x)
    x_mat = x.reshape(x.shape[0], -1).t()
    for i in range(len(x)):
        indexs = torch.tensor([x for j, x in enumerate(range(len(x))) if j != i]).to(x.device)
        others = x_mat.index_select(1, indexs)
        curr_x_perp = others @ torch.inverse(others.t() @ others) @ (others.t() @ x_mat[:, i])
        xperp[i] = curr_x_perp.reshape(x.shape[1:])
        # # Sanity check
        # for j in indexs:
        #     curr_x_perp = others @ torch.inverse(others.t() @ others) @ (others.t() @ x_mat[:, j])
        #     assert torch.allclose(curr_x_perp, x_mat[:, j], atol=1e-07), f"{i} {j.item()}"
    return xperp


def get_weights(x, sum2one=False):
    similarity_mat = get_l2_similarity_matrix(x)
    nn_dist, nn_i = torch.topk(similarity_mat, 2, dim=1, largest=False)
    min_dist = nn_dist[:, 1].reshape(-1, 1)
    max_dist = similarity_mat.max(dim=1)[0].reshape(-1, 1)
    dists = torch.div(similarity_mat - min_dist, max_dist - min_dist)
    dists[range(len(dists)), range(len(dists))] = torch.ones(similarity_mat.shape[0]).to(dists.device)
    weights = 1 - dists
    return weights if sum2one else weights / weights.sum(dim=1).reshape(-1, 1)  # normalize per row


def calc_Vt(x0_t, Sigma, H_funcs, U_t_y):
    V_t_x0_c = H_funcs.Vt(x0_t)
    SVt_x0_c = (V_t_x0_c * Sigma)[:, :U_t_y.shape[1]]
    return V_t_x0_c, SVt_x0_c


def efficient_generalized_steps(x, seq, model, b, H_funcs, y_0, sigma_0, etaB, etaA, etaC, cls_fn=None, classes=None,
                                guidance_eta=0, g_dist=None, num_timesteps=None):
    with torch.no_grad():
        ...
        init_y[:, large_singulars_index[0]] = U_t_y[:, large_singulars_index[0]] / \
        ...
        for i, j in tqdm(zip(reversed(seq), reversed(seq_next))):
            ...
            if guidance_eta != 0 and j != -1:
                guidance_factor = j / num_timesteps

                # Adapt each image in our xt batch according to its NN in the batch
                x0_4direction, nn_idx = get_nn(x0_t)
                ...
                x0_adjust.append(x0_t_g.to('cpu'))
            # # --------------
                V_t_x0_g = H_funcs.Vt(x0_t_g)
                et_g = (xt - x0_t_g * at.sqrt()) / (1 - at).sqrt()
            # # --------------
            ...
            diff_sigma_t_nextB = torch.sqrt(sigma_next ** 2 - sigma_0 ** 2 /
                                            singulars[cond_before_lite] ** 2 * (etaB ** 2))

            # missing pixels
            if guidance_eta != 0 and j != -1:
                Vt_xt_mod_next = V_t_x0_g + sigma_tilde_nextC * H_funcs.Vt(et_g) + \
                    std_nextC * torch.randn_like(V_t_x0_g)
            else:
            ...
            Vt_xt_mod_next[:, cond_after] = \
                V_t_x0[:, cond_after] + sigma_tilde_nextA * ((U_t_y - SVt_x0) / sigma_0)[:, cond_after_lite] + \
                std_nextA * torch.randn_like(V_t_x0[:, cond_after])

            # noisier than y (before)
            Vt_xt_mod_next[:, cond_before] = \
                (Sig_inv_U_t_y[:, cond_before_lite] * etaB + (1 - etaB) * V_t_x0[:, cond_before] +
    ...
    return xs, x0_preds, xs_adjust, x0_adjust
