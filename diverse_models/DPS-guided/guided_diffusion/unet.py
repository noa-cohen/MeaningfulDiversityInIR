...
def create_model(
    ...
    try:
        ...
        try:
            print('trying loading from DDRM version of UNET')
            from guided_diffusion.ddrm_unet import Model as DDRMUNetModel
            from typing import NamedTuple, List

            class DDRMUNetConfig(NamedTuple):
                ch: int  # 128
                out_ch: int  # 3
            ...
            model = DDRMUNetModel(DDRMUNetConfig(
                ch=num_channels,
                out_ch=3,
                ...
                if ds in attention_resolutions:
                    layers.append(
                        AttentionBlock(
                            ...
                            num_head_channels=num_head_channels,
                            use_new_attention_order=use_new_attention_order,
                        ) if not ddrm_attn else AttnBlock(ch)
                    )
