...
class EpsilonXMeanProcessor(MeanProcessor):
    ...
    def get_nn(self, x, gtk_mask):
        if gtk_mask is not None:
            gtk_mask = gtk_mask.repeat(x.shape[0], x.shape[1], 1, 1)
            mask = gtk_mask == 0.  
        else:
    ...
    def get_mean_and_xstart(self, x, t, model_output, g_dict):
        pred_xstart = self.predict_xstart(x, t, model_output)
        pred_xstart = self.process_xstart(pred_xstart)
        # # ---------------- 4guidance -----------------------
        g_eta = g_dict['g_eta']
        g_dist = g_dict['g_dist']
        if g_eta != 0 and t[0] != 0:
            guidance_factor = t[0] / g_dict['num_timesteps']

            if g_dict['is_inp']:
                x0_4direction, nn_idx = self.get_nn(pred_xstart, g_dict['mask'])
            else:
                x0_4direction, nn_idx = self.get_nn(pred_xstart, None)

            dist_f = torch.ones(pred_xstart.shape[0], 1, 1, 1).to(pred_xstart.device)
            if g_dist is not None:
                distance = torch.linalg.norm((pred_xstart - x0_4direction).reshape(pred_xstart.shape[0], -1), dim=1
                                             ) / g_dict['mask_size']
                # Hard thresholds
                # don't change images where normalized distance is larger than X
                dist_f[distance > g_dist] = 0
            pred_xstart = pred_xstart + (pred_xstart - x0_4direction) * g_eta * guidance_factor * dist_f
            # # ---------------------------------------------------
        pred_xstart = self.process_xstart(pred_xstart)
