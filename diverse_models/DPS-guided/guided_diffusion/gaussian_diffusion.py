...
class GaussianDiffusion:
    ...
    def p_sample_loop(self,
        ...
        for idx in pbar:
            ...
            out = self.p_sample(x=img, t=time, model=model, g_dict=g_dict)
    ...
    def p_mean_variance(self, model, x, t, g_dict):
        ...
        g_dict['num_timesteps'] = self.num_timesteps
        model_mean, pred_xstart = self.mean_processor.get_mean_and_xstart(x, t, model_output, g_dict)
...
class DDPM(SpacedDiffusion):
    def p_sample(self, model, x, t, g_dict):
        out = self.p_mean_variance(model, x, t, g_dict)
        sample = out['mean']

        noise = torch.randn_like(x)
        if t.sum() != 0:  # no noise when t == 0
